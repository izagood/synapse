//! MCP 사이드카 프로비저닝 + 브리지 접속 정보 조립 (GUI/PTY 비의존).
//!
//! 에이전트(claude/codex)가 Synapse MCP 사이드카를 자동 인식해 "지금 보는
//! 노트"를 받아 쓰도록, `.mcp.json`/codex 스니펫을 만들고 브리지 접속 정보를
//! 조립한다. 접속 정보 전달은 discovery 파일(bridge.json)이 담당한다(src-tauri).
