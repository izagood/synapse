...
describe("isCompositionBoundaryKey", () => {
  it("일반 키(엔터·스페이스·영문)는 조합 경계다", () => {
    expect(isCompositionBoundaryKey({ isComposing: false, keyCode: 13, key: "Enter" })).toBe(true);
    expect(isCompositionBoundaryKey({ isComposing: false, keyCode: 32, key: " " })).toBe(true);
    expect(isCompositionBoundaryKey({ isComposing: false, keyCode: 65, key: "a" })).toBe(true);
  });

  it("IME 처리 키(229/isComposing)는 경계가 아니다", () => {
    expect(isCompositionBoundaryKey({ isComposing: false, keyCode: 229, key: "Process" })).toBe(
      false,
    );
    expect(isCompositionBoundaryKey({ isComposing: true, keyCode: 65, key: "a" })).toBe(false);
  });

  it("수식 키는 자모 조합의 일부이므로 경계가 아니다", () => {
    for (const [keyCode, key] of [
      ...
      expect(isCompositionBoundaryKey({ isComposing: false, keyCode, key })).toBe(false);
    }
  });
});
...
describe("attachImeStabilizer — insertText 어댑터 (실기기 계측 시퀀스)", () => {
  ...
  it("Shift 조합 자모(ㅖ·ㄲ 등)의 수식 키 keydown이 조합을 끊지 않는다 — '계획해' 회귀", () => {
    const { term, textarea, sent } = makeTarget();
    attach(term);

    // 계 = ㄱ(r) + ㅖ(Shift+p). 수식 키 keydown은 IME 처리 키(229)가 아니라
    // 자기 keyCode(Shift=16)로 오고 isComposing도 false다 — 이걸 조합 경계로
    // 오판해 flush하면 시작 자모가 낱자로 새어 "ㄱ계획해"가 된다.
    fireKeydown(textarea, 229);
    fireInput(textarea, "beforeinput", "insertText", "ㄱ");
    fireKeydown(textarea, 16, false, "Shift"); // ← 조기 flush 금지
    fireKeydown(textarea, 229);
    fireInput(textarea, "beforeinput", "insertReplacementText", "계");
    expect(sent).toEqual([]); // "ㄱ"가 새지 않아야 한다

    // 획 시작 = 계 확정, 해 시작 = 획 확정, 엔터 = 해 확정
    fireInput(textarea, "beforeinput", "insertText", "ㅎ");
    fireInput(textarea, "beforeinput", "insertReplacementText", "획");
    fireInput(textarea, "beforeinput", "insertText", "ㅎ");
    fireInput(textarea, "beforeinput", "insertReplacementText", "해");
    fireKeydown(textarea, 13, false, "Enter");
    expect(sent).toEqual(["계", "획", "해"]);
  });
