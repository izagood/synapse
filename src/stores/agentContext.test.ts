...
describe("buildContextBlock", () => {
  it("root가 없으면 빈 문자열", () => {
    expect(buildContextBlock({ root: null, activePath: null, openPaths: [] })).toBe("");
  });

  it("열린 탭이 없으면 빈 문자열", () => {
    expect(buildContextBlock({ root: ROOT, activePath: null, openPaths: [] })).toBe("");
  });
...
describe("buildAgentPrompt", () => {
  it("컨텍스트가 없으면 프롬프트를 그대로 둔다 (회귀 방지)", () => {
    expect(
      buildAgentPrompt("안녕", { root: null, activePath: null, openPaths: [] }),
    ).toBe("안녕");
  });

  it("컨텍스트가 있으면 프롬프트 앞에 블록을 덧붙인다", () => {
    const out = buildAgentPrompt("이 노트 요약해줘", {
      root: ROOT,
      activePath: `${ROOT}/today.md`,
      openPaths: [`${ROOT}/today.md`],
    });
