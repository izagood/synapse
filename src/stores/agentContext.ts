...
export function buildContextBlock(
  ...
  if (activeRel) {
    lines.push(translate(language, "agent.ctxCurrentNote", { path: activeRel }));
  }
  // 여러 탭이 열려 있으면 활성 노트 외의 목록도 알려준다.
  const others = rels.filter((r) => r !== activeRel);
  if (others.length > 0) {
    const key = activeRel ? "agent.ctxOtherNotes" : "agent.ctxOpenNotes";
    lines.push(translate(language, key, { notes: others.join(", ") }));
  }
...
export function buildAgentPrompt(
  ...
  const block = buildContextBlock(input, language);
  if (!block) return prompt;
  return `${translate(language, "agent.ctxHeader")}\n${block}\n\n${prompt}`;
}
